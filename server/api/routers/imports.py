...
@router.get("/prospects_files/{upload_id}/progress", response_model=schemas.UploadStatusResponse)
...
def new_upload_request(
    ...
    # send file info to upload service with thread
    write_data = schemas.UploadWriteRequest(upload_id=upload.id, file_name=upload.file_name, force=force, has_headers=has_headers)
    t = ExcThread(target=post_to_upload_service, args=(UPLOAD_SVC_ENDPOINT, write_data))
    t.start()

    # return success response to user
    return JSONResponse({"success": True, "message": "Upload iniated!", "file_hash": file_info.get_file_hash(), "id": upload.id}, 201)
